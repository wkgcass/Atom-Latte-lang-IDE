( function( window, atom, require, module, undefined ) {
	...
	commands.activate = function( state ) {
		...
		atom.config.observe( 'atom-latte-lang-ide', {}, function( value ) {
            var latte=value.latte;
            if(latte==null||latte==undefined|| (typeof latte)!='string' || latte=='')
                latte="latte";
			...
			registeredAtomCommands = [];
                ...
                var matchsForCommands= [
                    "(?<text>.*) at (?<file>[\\/0-9a-zA-Z\\-\\\\._\\\\]+)\\((?<line>\\d+),(?<col>\\d+)\\)",
                    "(?<text>.*)\\((?<file>[\\/0-9a-zA-Z\\-\\\\._\\\\]+):(?<line>\\d+)\\)"
                ];
                value.commands = [
                    {
                        name: "Run Script",
                    ...
                    {
                        name: "Generate build.lts and run.lts",
                        command: latte,
                        arguments: [
                            "-gb",
                            "{ProjectDir}"
				...
				value.commands.forEach( function( command ) {
					...
					var commandName = 'atom-latte-lang-ide:' + command.name;
					...
					if (keymap) {
						...
						var keyname = 'atom-latte-lang-ide-keymap:' + command.name;
		...
		var prefix = 'atom-latte-lang-ide-config:';
	...
	function updateTitle(title) {
		...
		if (!title) {
			commands.panel.setTitle('Atom Latte-lang IDE');
		}	else {
			commands.panel.setTitle('Atom Latte-lang IDE: ' + title);
		}
	}
